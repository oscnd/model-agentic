...
// WebSearchArguments represents arguments for web search function
...
func main() {
	...
	webSearchDeclaration := function.NewDeclaration(
		gut.Ptr("web_search"),
		gut.Ptr("Search the web for information about a specific topic"),
		func(arguments *WebSearchArguments) (map[string]any, *gut.ErrorInstance) {
			...
			for _, value := range results {
				if strings.Contains(strings.ToLower(value), strings.ToLower(arguments.Query)) {
					result = value
					break
				}
			}
			...
			if result == "" {
				result = "Found general information about: " + arguments.Query
			}
	...
	state.FunctionState.OnBeforeFunctionCall = func(callback *function.CallbackBeforeFunctionCall) (any, *gut.ErrorInstance) {
